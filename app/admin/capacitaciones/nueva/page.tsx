        ...
        <div className="flex items-center justify-center gap-1 sm:gap-2">
          {steps.map((step, i) => (
            <div key={step.number} className="flex items-center gap-1 sm:gap-2">
              <div className="flex items-center gap-1.5 sm:gap-2">
                <div
                  className={`flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full text-sm font-medium transition-colors shrink-0 ${
                    currentStep >= step.number ? "bg-[#2d4a8a] text-white" : "bg-[#eaf0fb] text-[#a5a9b8]"
                  }`}
                >
                ...
                <span
                  className={`hidden sm:inline text-sm font-medium ${
                    currentStep >= step.number ? "text-[#15182b]" : "text-[#a5a9b8]"
                  }`}
                >
                  {step.label}
                </span>
              ...
              {i < steps.length - 1 && (
                <div className={`w-8 sm:w-16 h-px ${currentStep > step.number ? "bg-[#2d4a8a]" : "bg-[#e8e4dc]"}`} />
              )}
            </div>
          ))}
        </div>
        <p className="sm:hidden text-center text-sm font-medium text-[#15182b]">
          ...
          <Card className="border-[#e8e4dc] shadow-sm">
            <CardContent className="p-6 space-y-5">
              <div className="space-y-2">
                <Label htmlFor="title" className="text-sm font-medium text-[#15182b]">
              ...
              <div className="space-y-2">
                <Label htmlFor="description" className="text-sm font-medium text-[#15182b]">
                  Descripción
                </Label>
                <textarea
                  id="description"
            ...
            <CardContent className="p-6 space-y-5">
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <LinkIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#a5a9b8]" aria-hidden="true" />
                    <Input
                  ...
                  <button
                    ...
                    className="h-11 px-4 rounded-lg bg-[#2d4a8a] text-white text-sm font-medium hover:bg-[#15182b] transition-colors disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
                  >
                    Agregar
                  </button>
                </div>
                ...
                {videoUrl.trim() && (
                  <div className="rounded-lg border border-[#e8e4dc] overflow-hidden bg-[#f6f3ee]">
                    {previewEmbed ? (
                      ...
                      <div className="flex items-center gap-2 p-3 text-sm text-[#a5a9b8]">
              ...
              {videoLinks.length > 0 ? (
                  ...
                  {videoLinks.map((v) => (
                    <div key={v.id} className="flex items-center gap-3 p-3 rounded-lg border border-[#e8e4dc] bg-[#f6f3ee]">
                      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#eaf0fb] shrink-0">
                        <Video className="h-4 w-4 text-[#6b7185]" aria-hidden="true" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-[#15182b] truncate">
                ...
                <div className="flex flex-col items-center py-4 text-center">
                  <FileText className="h-8 w-8 text-[#e8e4dc] mb-2" aria-hidden="true" />
                  <p className="text-sm text-[#6b7185]">No hay material cargado todavía.</p>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6 space-y-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-[#6b7185] mt-0.5">
                ...
                >
                  <span
                    className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-[#f6f3ee] shadow transition-transform ${
                ...
                <>
                  ...
                  <div className="space-y-4">
                      ...
                      <div key={q.id} className="p-4 rounded-lg border border-[#e8e4dc] space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          {questions.length > 1 && (
                            <button
                              ...
                              className="text-[#a5a9b8] hover:text-red-500 transition-colors"
                            >
                        ...
                        <div className="space-y-2">
                            ...
                            <div key={opt.id} className="flex items-center gap-2">
                              <button
                                ...
                                className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${
                                  opt.isCorrect ? "border-[#2d4a8a] bg-[#2d4a8a]" : "border-[#e8e4dc] hover:border-slate-400"
                                }`}
                              >
                          ...
                          <button
                            ...
                            className="text-sm text-[#2d4a8a] hover:text-[#15182b] font-medium"
                          >
                            + Agregar opción
                          </button>
                        </div>
                    ...
                    <button
                      type="button"
                      onClick={addQuestion}
                      className="flex items-center gap-2 text-sm text-[#2d4a8a] hover:text-[#15182b] font-medium"
                    >
        ...
        <div className="flex items-center justify-between">
          ...
          {currentStep === 3 && (
            <button
              ...
              onClick={handleSave}
              disabled={saving}
              className="inline-flex items-center gap-2 h-11 px-5 rounded-lg bg-[#2d4a8a] text-white text-sm font-medium hover:bg-[#15182b] transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {saving ? (
                <>
