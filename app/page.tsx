...
const styles = {
  ...
  statLabel: { fontSize: 12, color: COLORS.textMuted, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.08em" },
  ...
  fileList: { display: "flex", flexDirection: "column", gap: 12, marginBottom: 24 },
  fileItem: (active) => ({
    display: "flex",
    alignItems: "center",
    gap: 16,
...
function Sidebar({ rol, active, onNav, onLogout }) {
  const adminNav = [
    ...
    <div style={styles.main}>
      ...
      <div style={styles.card}>
        ...
        <table style={styles.table}>
          <thead>
            <tr>
              {["Título", "Área", "Estado", "Progreso", ""].map((h) => (
          ...
          <tbody>
            {capacitaciones.map((c) => (
              <tr key={c.id}>
                ...
                <td style={styles.td}>
                  <span style={styles.badge(c.estado === "Publicado" ? "green" : "yellow")}>{c.estado}</span>
                </td>
                <td style={styles.td}>
                  {c.colaboradores > 0 ? (
                ...
                <td style={styles.td}>
                  <button style={styles.btnGhost}>Ver detalle</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div style={styles.card}>
        ...
        <table style={styles.table}>
          <thead>
            <tr>
              {["Nombre", "Área", "Estado", "Última nota"].map((h) => (
                <th key={h} style={styles.th}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {colaboradores.map((c) => (
              <tr key={c.nombre}>
                ...
                <td style={styles.td}><span style={{ color: COLORS.textMuted }}>{c.area}</span></td>
                <td style={styles.td}>
                  <span style={styles.badge(c.estado === "Completado" ? "green" : c.estado === "En progreso" ? "purple" : "yellow")}>
                    {c.estado}
                  </span>
                </td>
                <td style={styles.td}>
...
function CapacitacionesPage({ onVerCapacitacion }) {
  return (
    <div style={styles.main}>
      ...
      <div style={styles.card}>
        <table style={styles.table}>
          <thead>
            <tr>
              {["Título", "Área", "Estado", "Asignados", "Completados", ""].map((h) => (
                <th key={h} style={styles.th}>{h}</th>
              ))}
            </tr>
          </thead>
...
function TrainingView({ cap, onStartQuiz, onBack }) {
    ...
    <div style={styles.main}>
      <div style={styles.quizWrap}>
        ...
        <div style={styles.questionCard}>
          <div style={{ fontSize: 17, fontWeight: 600, lineHeight: 1.6, marginBottom: 24 }}>
            {q.texto}
          </div>
          {q.opciones.map((op, i) => (
...
function ResultPage({ score, onBack }) {
  const passed = score >= 60;
  return (
    <div style={styles.main}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "60vh" }}>
        <div style={styles.resultCard}>
          <div style={styles.resultCircle(passed)}>
            {passed ? "✓" : "✗"}
          </div>
...
function PortalHome({ onVerCapacitacion }) {
  const misCapacitaciones = [
    { id: 1, titulo: "Protocolo de Higiene Personal", area: "Cuidado", estado: "En progreso", progreso: 66, fechaLimite: "31 mar" },
    { id: 2, titulo: "Manejo de Emergencias Médicas", area: "Enfermería", estado: "Pendiente", progreso: 0, fechaLimite: "15 abr" },
  ];
  return (
    <div style={styles.main}>
...
export default function App() {
  ...
  const handleVerCapacitacion = (cap) => {
    setSelectedCap(cap);
  ...
  if (activeNav === "training" && selectedCap) {
    ...
    else if (activeNav === "colaboradores") content = (
      <div style={styles.main}>
